...
class CNN_Logical(BasicModel):
    ...
    def similarity_batch(self, vector1, vector2, sigmoid=False):
        result = F.cosine_similarity(vector1, vector2, dim=0)
        result = result * 100
        if sigmoid:
            return result.sigmoid()
        return result  
...
#         print(image_features.shape)
        ...
        choice1_1 = self.logic_and(row1,row3_1)
        choice1_2 = self.logic_and(row2,row3_1)
        choice1_3 = self.logic_and(row1_row2,row3_1)
        constraints.extend([row3_1,choice1_1,choice1_2,choice1_3])
        
        row3_2 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,9])
        choice2_1 = self.logic_and(row1,row3_2)
        choice2_2 = self.logic_and(row2,row3_2)
        choice2_3 = self.logic_and(row1_row2,row3_2)
        constraints.extend([row3_2,choice2_1,choice2_2,choice2_3])
        
        row3_3 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,10])
        choice3_1 = self.logic_and(row1,row3_3)
        choice3_2 = self.logic_and(row2,row3_3)
        choice3_3 = self.logic_and(row1_row2,row3_3)
        constraints.extend([row3_3,choice3_1,choice3_2,choice3_3])
        
        row3_4 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,11])
        choice4_1 = self.logic_and(row1,row3_4)
        choice4_2 = self.logic_and(row2,row3_4)
        choice4_3 = self.logic_and(row1_row2,row3_4)
        constraints.extend([row3_4,choice4_1,choice4_2,choice4_3])
        
        row3_5 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,12])
        choice5_1 = self.logic_and(row1,row3_5)
        choice5_2 = self.logic_and(row2,row3_5)
        choice5_3 = self.logic_and(row1_row2,row3_5)
        constraints.extend([row3_5,choice5_1,choice5_2,choice5_3])
        
        row3_6 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,13])
        choice6_1 = self.logic_and(row1,row3_6)
        choice6_2 = self.logic_and(row2,row3_6)
        choice6_3 = self.logic_and(row1_row2,row3_6)
        constraints.extend([row3_6,choice6_1,choice6_2,choice6_3])
        
        row3_7 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,14])
        choice7_1 = self.logic_and(row1,row3_7)
        choice7_2 = self.logic_and(row2,row3_7)
        choice7_3 = self.logic_and(row1_row2,row3_7)
        constraints.extend([row3_7,choice7_1,choice7_2,choice7_3])
        
        row3_8 = self.create_row(image_features[:,6],image_features[:,7],image_features[:,15])
        choice8_1 = self.logic_and(row1,row3_8)
        choice8_2 = self.logic_and(row2,row3_8)
        choice8_3 = self.logic_and(row1_row2,row3_8)
        constraints.extend([row3_8,choice8_1,choice8_2,choice8_3])
...
#             print(constraints.shape)
        ...
        prediction_c1 = (self.similarity(choice1_1, self.true_vector).view([-1]) 
                         +self.similarity(choice1_2, self.true_vector).view([-1]) 
                         +self.similarity(choice1_3, self.true_vector).view([-1]))
        prediction_c2 = (self.similarity(choice2_1, self.true_vector).view([-1]) 
                         + self.similarity(choice2_2, self.true_vector).view([-1]) 
                         + self.similarity(choice2_3, self.true_vector).view([-1]))
        prediction_c3 = (self.similarity(choice3_1, self.true_vector).view([-1]) 
                         + self.similarity(choice3_2, self.true_vector).view([-1]) 
                         + self.similarity(choice3_3, self.true_vector).view([-1]))
        prediction_c4 = (self.similarity(choice4_1, self.true_vector).view([-1]) 
                         +self.similarity(choice4_2, self.true_vector).view([-1]) 
                         + self.similarity(choice4_3, self.true_vector).view([-1]))
        prediction_c5 = (self.similarity(choice5_1, self.true_vector).view([-1]) 
                         +self.similarity(choice5_2, self.true_vector).view([-1]) 
                         + self.similarity(choice5_3, self.true_vector).view([-1]))
        prediction_c6 = (self.similarity(choice6_1, self.true_vector).view([-1]) 
                         +self.similarity(choice6_2, self.true_vector).view([-1]) 
                         + self.similarity(choice6_3, self.true_vector).view([-1]))
        prediction_c7 = (self.similarity(choice7_1, self.true_vector).view([-1]) 
                         +self.similarity(choice7_2, self.true_vector).view([-1]) 
                         + self.similarity(choice7_3, self.true_vector).view([-1]))
        prediction_c8 = (self.similarity(choice8_1, self.true_vector).view([-1]) 
                         +self.similarity(choice8_2, self.true_vector).view([-1]) 
                         + self.similarity(choice8_3, self.true_vector).view([-1]))
#         print(constraints.shape)
